...
class SignInActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnRegistro.setOnClickListener {
            ...
            }else if (passwordUsuario.isEmpty() || !passwordRegex.matcher(passwordUsuario).matches()){
                Toast.makeText(this, "La contraseña debe contener mínimo 4 caracteres, incluyendo: Mínimo 1 letra mayúscula, 1 letra minúscula y 1 número",
                    Toast.LENGTH_LONG).show()
            }else if (passwordUsuario != passwordUsuario2){
                Toast.makeText(this, "Las contraseñas introducidas NO coinciden",
                    Toast.LENGTH_SHORT).show()
            }else{
        ...
        auth.createUserWithEmailAndPassword(email, password)
            .addOnCompleteListener(this) { task ->
                if (task.isSuccessful) {
                    ...
                    Toast.makeText(this, "El correo electrónico introducido ya está en uso",
                        Toast.LENGTH_SHORT).show()
                }
            }
    }
